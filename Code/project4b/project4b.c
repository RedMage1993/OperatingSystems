...
//
// Difference from a is report on max channel
// size. This tests if a n-queue is required.
//********************************************
...
uint read(procNode *proc);
void write(procNode *proc, uint uId);
void * phase(void *arg);
...
int main()
{
	...
	for (iterator = 0; iterator < size; iterator++)
	{
	    printf("[%u] channel max = %u\n", procs[iterator].uId, procs[iterator].chMaxSize);
...
uint read(procNode *proc)
{
	...
	if (sem_wait(&proc->sync))
		return (uint) pError("Bad sem_wait from read");
	if (sem_wait(&proc->crit))
		return (uint) pError("Bad sem_wait from read");

	// Critical section begin.
	uId = proc->channel->uId;

	next = proc->channel->next; // Backup channel's next before deleting.

	free(proc->channel);

	proc->channel = next; // Update queue's head.

	proc->chSize--;
	// Critical section end.

	if (sem_post(&proc->crit))
...
void write(procNode *proc, uint uId)
{
	...
	if (!proc->channel)
		proc->channel = new; // Has no head (at least tempUId has been read).
	else
	{
		iterator = proc->channel;
	...
	proc->chSize++;
	if (proc->chSize > proc->chMaxSize)
	    proc->chMaxSize = proc->chSize;
	// Critical section end.
	...
	if (sem_post(&proc->sync)) // Increment sync sem (so read can now proceed).
		pError("Bad sem_post from write");
}
...
void * phase(void *arg)
{
	...
	while (!done)
	{
		...
		if (tArg->proc->active)
		{
			...
			write(tArg->nextProc, tArg->proc->tempUId);
			oneHop = read(tArg->proc);
			write(tArg->nextProc, oneHop);

			twoHop = read(tArg->proc);
